...
def function6():
    ...
    arr = np.full((10),np.nan,)#wrtie your code here
    arr[4]=10
    arr[7]=20
    return arr
...
function6()
    """
    Expected Output:
          array([nan, nan, nan, nan, 10., nan, nan, 20., nan, nan])
    """ 
...
def function10():
    ...
    brr = np.array([1,2,3])
    subt = np.subtract(arr,brr.repeat(3).reshape(3,3))
...
def function11():
    ...
    arr[arr%2==1] = -1 #write your code here 
    ans=arr
...
def function12():
    ...
    arr = np.array([1,2,3])
    ans = arr.repeat(3)#write your code here 
    ans = np.concatenate((ans, arr, arr,arr))
    return ans
function12()
     """
     Expected Output:
...
def function17():
    ...
    ans = np.where((arr%3==0) & (arr%5==0), "YES", arr)
    ans = np.where((arr%3!=0) | (arr%5!=0), "NO", ans)
...
def function18():
    ...
    common = np.intersect1d(piaic, students) 
    x = common.size # Write you code Here
    return x
function18()
